...
class VolumeRenderer(torch.nn.Module):
    ...
    def _compute_weights(
        ...
        cumprod_input = torch.cat([ones, (1 - alpha + eps)], dim=1)
        T = torch.cumprod(cumprod_input, dim=1) 
        T = T[:, :-1, :] 
    ...
    def forward(
        ...
        for chunk_start in range(0, B, self._chunk_size):
            ...
            feature = self._aggregate(weights= weights, rays_feature= rays_feature)
            acc_transmittance = weights.sum(dim=1)
            if self._white_background:
                feature = feature + (1 - acc_transmittance)
...
class SphereTracingRenderer(torch.nn.Module):
    ...
    def sphere_tracing(
        ...
        sdf_list = implicit_fn(points)
        mask = (sdf_list <= threshold)
        return points, mask 
...
class VolumeSDFRenderer(VolumeRenderer):
    ...
    def forward(
        ...
        for chunk_start in range(0, B, self._chunk_size):
            ...
            density = sdf_to_density(distance, alpha=self.alpha, beta=self.beta) # TODO (Q7): convert SDF to density
