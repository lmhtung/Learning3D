...
class NeuralRadianceField(torch.nn.Module):
    def __init__(
        ...
        self.dir_layers = torch.nn.ModuleList([torch.nn.Linear(embedding_dim_dir + self.n_neurons_xyz, self.n_neurons_dir)]+
                                              [torch.nn.Linear(self.n_neurons_xyz, self.n_neurons_dir) for i in range(self.n_dir_layers - 1)])
    ...
    def forward(self, ray_bundle):
        ...
        xyz = self.harmonic_embedding_xyz(sample_points)
        add = xyz.clone()
        for i in range(self.n_xyz_layers):
            xyz = F.relu(self.n_xyz_layers[i](xyz))
            if i in self.concat:
                xyz = torch.cat([xyz, add], dim= -1) 
        
        density = F.relu(self.density_layer(xyz))

        directions = ray_bundle.directions.unsqueeze(1).expand(-1, xyz.shape[0], -1)
        dir = self.harmonic_embedding_dir(directions)

        input = torch.cat([xyz, dir], dim= -1)
        for i in range(self.n_dir_layers):
            input = F.relu(self.dir_layers[i](input))
        rgb = F.sigmoid(self.rgb_layer(input))
