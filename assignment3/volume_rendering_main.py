...
def render_images(
    ...
    for cam_idx, camera in enumerate(cameras):
        ...
        if cam_idx == 0 and file_prefix == '':
            ...
            points = pts.view(1, -1, 3)  # (1, N_total, 3)
        ...
        if cam_idx == 2 and file_prefix == '':
            depth = out['depth'].detach().cpu().reshape(image_size[1], image_size[0]).numpy() # w, h
...
def train(
    ...
    for epoch in t_range:
        for iteration, batch in enumerate(train_dataloader):
            ...
            loss = torch.mean((out['feature'] - rgb_gt) **2) # OG
...
def train_nerf(
    ...
    for epoch in range(start_epoch, cfg.training.num_epochs):
        ...
        for iteration, batch in t_range:
            ...
            loss = torch.mean((out['rgb'] - rgb_gt) **2)
