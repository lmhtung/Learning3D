...
def evaluate(predictions, mesh_gt, thresholds, args):
    if args.type == "vox":
        ...
        print("num occupied voxels:", (voxels_src > 0.1).sum())
...
def evaluate_model(args):
    ...
    for step in range(start_iter, max_iter):
        ...
        """
        if (step % args.vis_freq) == 0:
            rend = predictions.detach().cpu().numpy()[0]  # [H, W, D]
            # lấy lát cắt giữa theo trục D
            mid_slice = rend[:, :, rend.shape[2] // 2]
            # lưu trực tiếp bằng imsave với màu xanh lam
            plt.imsave(f"vis/{step}_{args.type}.png", mid_slice, cmap="Blues")
        """
